...
export function sonderHeikhal(renderer, scene, { kelim, materiaux, eteints }) {
  ...
  const allumes = eteints.filter((o) => o.visible);
  const [ciel, brume] = [scene.environment, scene.fog];
  allumes.forEach((o) => { o.visible = false; });
  scene.environment = null;
  scene.fog = null;
  camera.update(renderer, scene);
  allumes.forEach((o) => { o.visible = true; });
  scene.environment = ciel;
  scene.fog = brume;
