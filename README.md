...
| `beit_hamikdash_gravures.py` | Les trois figures gravées des parois du Bayit — keruv, timora, fleuron. Le script dessine leurs guides (composition et iconographie, en dômes et sillons), les fait tailler par gpt-image-2 (fal.ai) dans la manière de l'ornement judéen du Second Temple — ossuaires, portes de 'Houlda — — les tuiles taillées, `gravures/*.png`, sont la source versionnée — et en compose l'atlas `visite/matieres/gravures_2048.webp` (luminance = modelé, distance au bord = volume), plus `gravures.json` : la silhouette de chaque figure, tracée sur la carte même. Le blockout ne pose plus qu'une plaque par figure, à cette silhouette, dont les UV visent la tuile ; la matière et le nuanceur lisent le modelé. À relancer après toute modification, puis reconstruire. |
