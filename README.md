...
| `beit_hamikdash_gravures.py` | Les trois figures gravées des parois du Bayit — keruv, timora, fleuron. Le script dessine leurs guides (composition et iconographie, en dômes et sillons), les fait tailler par gpt-image-2 (fal.ai) dans la manière de l'ornement judéen du Second Temple — ossuaires, portes de 'Houlda — — les tuiles taillées, `gravures/*.png`, sont la source versionnée — et en compose l'atlas `visite/matieres/gravures_2048.webp` (luminance = modelé, distance au bord = volume), plus `gravures.json` : la silhouette de chaque figure, tracée sur la carte même. Le blockout taille chaque figure à cette silhouette, 0,05 ama sous le nu : le fond de la taille a des UV qui visent la tuile, et le support perd le trou par un booléen ; la matière et le nuanceur lisent le modelé. À relancer après toute modification, puis reconstruire. |
| `gravures/` | Les trois tuiles taillées (`keruv.png`, `timora.png`, `fleuron.png`), sources de l'atlas, et leurs guides dans `guides/`. |
