...
def lit_plan(texte, plan):
    ...
    prompt = (f'{mouvement}. {bloc_commun(texte, "FIGURES")}'
              f' {bloc_commun(texte, "PLACES")}')
    return camera_du_plan(section), prompt, negatif_du_plan(texte, section)
...
def main():
    ...
    texte = texte_prompts()
    camera, mouvement, negatif = lit_plan(texte, args.plan)
    depart, fin = images_du_plan(camera, args.depart, args.fin)
    prompt = args.prompt or mouvement

    print(f"plan {args.plan} · {camera} · {args.modele} · {duree} s "
          f"· ~{duree * modele['prix_seconde']:.2f} $")
    print(f"  prompt  : {prompt}")
    print(f"  images  : {os.path.relpath(depart, RACINE)} → {os.path.relpath(fin, RACINE)}")

    if args.simulation:
        print(json.dumps(modele["charge"]("<depart>", "<fin>", prompt, negatif, duree, args.seed),
    ...
    cle = cle_api()
    corps = modele["charge"](televerse(depart, cle), televerse(fin, cle),
